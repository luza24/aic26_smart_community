  ...
  public:
      IntegratedNavigator()
      {
          ...
          safety_distance_ = 1.0;      // 进入避障的前向距离
          exit_distance_ = 1.5;        // 退出避障的前向距离
          emergency_distance_ = 0.4;   // 紧急距离：停车转向
          max_curvature_ = 3.0;

          avoid_linear_vel_ = 0.1;     // 绕障时的慢速前进速度
          avoid_turn_vel_   = 0.6;     // 绕障时的固定转向角速度
          clear_hold_time_  = 0.5;     // 前方需持续开阔多久才退出避障

          has_odom_ = false;
          has_path_ = false;
          avoiding_ = false;
          front_clearance_ = 999.0;
          clear_since_ = ros::Time(0);

          ROS_INFO("Integrated Navigator (PurePursuit + Simple Obstacle Avoidance) Started.");
      }
      ...
      void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
      {
          ...
          for (int i = 0; i < num_ranges; i++) {
              ...
              // 只看前方正负 0.5 rad（约 ±29°）扇形区域
              if (std::abs(angle) <= 0.5) {
                    front_clearance_ = std::min(front_clearance_, static_cast<double>(scan->ranges[i]));
              }
          }
      }
  ...
  private:
      ...
      geometry_msgs::Pose2D findLookahead() {
          if (path_.poses.empty()) {
              ...
              last.x = robot_pose_.x; last.y = robot_pose_.y; last.theta = 0;
              return last;
          }
      ...
      geometry_msgs::Twist computePurePursuit() {
          ...
          if (local_x < 0.2) {
              // 目标在后方：原地转向，方向由目标所在的左/右决定
              cmd.linear.x = 0.0;
              cmd.angular.z = (local_y >= 0.0) ? 0.5 : -0.5;
          ...
          cmd.linear.x = linear;
          cmd.angular.z = linear * curvature;
          return cmd;
      }
      ...
      void applyObstacleAvoidance(geometry_msgs::Twist& cmd) {
          ...
          if (!avoiding_) {
              if (front_clearance_ < safety_distance_) {
                  avoiding_ = true;
                  clear_since_ = ros::Time(0);   // 重置“持续开阔”计时
                  ROS_INFO("Enter obstacle avoidance.");
              }
          }

          if (!avoiding_) return;

          // ---- 2. 绕障：始终朝固定方向转弯 + 慢速前进，绕一个圆弧走（而不是原地转）----
          if (front_clearance_ < emergency_distance_) {
              // 太近：停车，稍快转向，先把车头从障碍物上转开
              cmd.linear.x = 0.0;
              cmd.angular.z = turn_direction_ * avoid_turn_vel_ * 1.5;
          } else {
              // 前方还有空间：一边前进一边朝固定方向绕
              cmd.linear.x = avoid_linear_vel_;
              cmd.angular.z = turn_direction_ * avoid_turn_vel_;
          }

          // ---- 3. 退出：前方“持续”开阔一段时间才恢复纯追踪，防止刚转开就转回来打摆 ----
          if (front_clearance_ > exit_distance_) {
              if (clear_since_.toSec() == 0.0) {
                  clear_since_ = ros::Time::now();
              } else if ((ros::Time::now() - clear_since_).toSec() > clear_hold_time_) {
                  avoiding_ = false;
                  ROS_INFO("Exit obstacle avoidance.");
              }
          } else {
              clear_since_ = ros::Time(0);   // 前方又近了，重新计时
          }

          cmd.angular.z = std::max(-max_angular_vel_, std::min(max_angular_vel_, cmd.angular.z));
          cmd.linear.x  = std::max(0.0, std::min(max_linear_vel_, cmd.linear.x));

          ROS_DEBUG_THROTTLE(1, "Avoiding: front=%.2f, turn=%.2f, lin=%.2f",
                             front_clearance_, cmd.angular.z, cmd.linear.x);
      }
      ...
      double safety_distance_, emergency_distance_, exit_distance_;
      double max_curvature_;

      double avoid_linear_vel_, avoid_turn_vel_, clear_hold_time_;
      ros::Time clear_since_;   // 前方持续开阔的起始时刻

      int turn_direction_ = -1;   // 绕障方向：1 = 左转，-1 = 右转（固定，不会来回摇摆）
